...
export default async function fetchGoogleTrends(
  ...
  const response = await fetch(
    `https://trends.google.com/trends/api/dailytrends?hl=en-US&tz=-120&geo=US&ns=15`,
  );

  // Ensure request was successful
  if (!response.ok) {
    throw new Error(
      `Failed to fetch Google Trends. Status: ${response.status}`,
    );
  }
  ...
  const trends =
    JSON.parse(trendsData).default.trendingSearchesDays[0].trendingSearches;

  // Extract the required number of trends
  const selectedTrends = trends.slice(0, count).map((
    trend: { title: { query: string }; formattedTraffic: string },
  ) => ({
